...
export class GenerateTokenImpl implements GenerateToken {
  ...
  async generateToken(commandOptions: GenerateTokenOptions, notifyStatus: (status: string) => void = () => {
    ...
    const loginUrl = this.buildLoginUrl(commandOptions);
    const url = this.buildUserProfileUrl(commandOptions);
  ...
  buildLoginUrl({url}: {url: string}): string {
    return `${url}/login`;
  }

  buildUserProfileUrl({url, username}: {url: string, username: string}): string {
    return `${url}/user/${username}/configure`;
  }
